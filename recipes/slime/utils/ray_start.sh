...
# strict: only pods named like <keyword>-<ordinal>-<suffix> (e.g. cyx-slime-sft-0-xxx).
#         So cyx-slime does NOT merge cyx-slime-rl / cyx-slime-sft (use full prefix: cyx-slime-sft).
# loose:  legacy grep "^keyword" (any pod name starting with keyword).
RAY_POD_MATCH="${RAY_POD_MATCH:-strict}"
...
log_info "RAY_POD_MATCH=$RAY_POD_MATCH (strict = job prefix + ordinal, e.g. cyx-slime-sft-0-xxx)"
...
for KEYWORD in "${JOB_KEYWORDS[@]}"; do
    # Filter pods: strict avoids merging cyx-slime with cyx-slime-rl / cyx-slime-sft (use full prefix).
    if [ "$RAY_POD_MATCH" = "loose" ]; then
        CURRENT_JOB_PODS=($(echo "$ALL_PODS_RAW" | grep "^$KEYWORD" | sort -V))
    else
        CURRENT_JOB_PODS=($(echo "$ALL_PODS_RAW" | grep -E "^${KEYWORD}-[0-9]+-" | sort -V))
    fi
