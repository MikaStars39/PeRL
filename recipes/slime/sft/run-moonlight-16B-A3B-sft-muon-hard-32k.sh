...
# Moonlight-16B-A3B SFT with hard context adaptation (8k -> 32k)
# No RoPE modification — the model learns to handle longer positions
# via gradient updates alone (brute-force extrapolation).
#
...
CKPT_ARGS=(
   ...
   --save /jpfs-5p/chenyanxu.9/model/Moonlight-16B-A3B-dolci-think-hard-32k
   --save-interval 128
)

# No YARN_ARGS — keep rotary_scaling_factor=1 (default from moonlight.sh).
# The model will directly extrapolate RoPE beyond 8192 positions.
...
WANDB_ARGS=(
   ...
   --wandb-group Moonlight-16B-A3B-sft-muon-hard-32k
)
